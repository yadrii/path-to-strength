            ...
            <CardContent className="p-6 space-y-4">
              ...
              <div className="flex gap-2">
                <Button
                  onClick={startVoice}
                  variant="outline"
                  className={`rounded-2xl h-14 w-20 ${isListening ? "bg-red-50 text-red-500 animate-pulse" : ""}`}
                >
          ...
          <ScrollArea className="h-full pr-4">
            <div className="space-y-6 pb-20">
              {feed.map((story) => (
                <Card
                  key={story.id}
                  className="border-none shadow-md rounded-[2rem] bg-white p-8 group transition-all"
                >
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex items-center gap-1.5 text-orange-500 text-xs font-bold">
                      <Flame
                        className={`h-4 w-4 ${story.diyas > 0 ? "fill-orange-500 animate-pulse" : ""}`}
                      />{" "}
                      {story.diyas} Diyas
                    </div>
                  </div>
