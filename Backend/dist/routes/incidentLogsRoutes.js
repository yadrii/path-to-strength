...
router.get("/", verifyToken, async (req, res) => {
    try {
        ...
        const formattedIncidents = incidents.map(incident => ({
            ...
            userName: incident.isAnonymous ? "Anonymous" : incident.userId?.name || "Unknown",
...
router.post("/:id/like", verifyToken, async (req, res) => {
    try {
        ...
        const likeIndex = incident.likes.indexOf(userId);
        if (likeIndex > -1) {
            // Unlike
...
router.delete("/:id/comments/:commentId", verifyToken, async (req, res) => {
    try {
        ...
        const commentIndex = incident.comments.findIndex(comment => comment._id?.toString() === commentId && comment.userId.toString() === userId);
        if (commentIndex === -1) {
...
//# sourceMappingURL=incidentLogsRoutes.js.map